"""
Perform an `rcc cloud authorize` command, grabbing the CR workspace ID from
the given JSON file and then setting the generated token to this JSON file.
...
execute `rcc cloud authorize` with this workspace ID. Then, it saves the
generated token to the `RC_API_TOKEN_V1` key in `devdata/env-producer.json`.
"""
