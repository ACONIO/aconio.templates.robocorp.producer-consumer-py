...
def MaxFailedGuard(
    max_fail: int,
    notification_recipients: str | list[str] | None = None,
    ...
    try:
        ...
        step_runs = cr.step_run.list_step_runs(
            workspace_id=workspace_id, process_run_id=process_run_id
        )

        # Here, we pass `max_fail - 1`, because the current step-run
        # is also failed and therefore counts towards failed step-runs.
        if _is_stop_process(step_runs, max_fail - 1):
            cr.process_run.stop_process_run(workspace_id, process_run_id)

            if notification_recipients:
                _send_notification_mail(
                    notification_recipients, workspace_id, process_run_id
                )
...
def _send_notification_mail(
    notification_recipients: str | list[str],
    workspace_id: str,
    process_run_id: str,
) -> None:
    """Send a notification mail to the specified email address(es).

    Args:
        notification_recipients:
            E-Mail address(es) to send the notification mail to.
    """
    workspace_info = cr.workspace.get_workspace(workspace_id)
