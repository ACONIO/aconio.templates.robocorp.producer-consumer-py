...
def cp_by_identifier(
    ...
    if rows:
        return [dao.ContactPerson(**r) for r in rows]
...
def email_by_adressart(
    ...
    """Retrieve the email of the given `client_bmd_id` with an addresstype."""
...
def responsible_employees(
    ...
    if rows:
        return [dao.Employee(**r) for r in rows]
